...
- (void)setAdList:(NSArray *)adList {
    NSMutableArray *tempArray = [NSMutableArray arrayWithArray:adList];
    NSMutableArray *tempArray2= [NSMutableArray array];
    if (tempArray.count > 4) {
        [tempArray2 addObjectsFromArray:[tempArray subarrayWithRange:NSMakeRange(0, 4)]];
    }else {
        tempArray2 = tempArray;
    }
    if (tempArray2.count > 0) {
        [tempArray2 insertObject:tempArray2.lastObject atIndex:0];
        [tempArray2 addObject:tempArray2[1]];
        _adList = tempArray2;
        ...
        self.pageControl.numberOfPages = tempArray2.count - 2;
        [self.timer fire];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SYHomeNewsModel *model = self.adList[indexPath.item];
...
- (UIPageControl *)pageControl {
    if (_pageControl == nil) {
        ...
        _pageControl.currentPageIndicatorTintColor = [UIColor appMainColor];
